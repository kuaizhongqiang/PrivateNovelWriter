...
fn active_novel_id(conn: &rusqlite::Connection) -> Result<String, String> {
    ...
    list.first()
        .map(|n| n.id.clone())
        .ok_or_else(|| "No novels".to_string())
}
...
fn get_chapter(state: State<AppState>, chapter_id: String) -> Result<serde_json::Value, String> {
    ...
    let content =
        pnw_kernel::storage::read_text(&full_path).map_err(|e| format!("Read error: {}", e))?;
            ...
            let result: Result<String, String> = match project_path {
                Some(path) => {
                    ...
                    let cmd = match command_type.as_str() {
                        ...
                        "revise" => {
                            let cid =
                                chapter_id.ok_or_else(|| "chapter_id required".to_string())?;
                            AgentCommand::ReviseChapter {
                                chapter_id: cid,
                                feedback: message,
                            }
                        }
                        "plan" => AgentCommand::PlanOutline {
                            novel_id,
                            brief: message,
                        },
                        _ => {
                            let cid = chapter_id
                                .or_else(|| {
                                    crud::list_outline_phases(&conn, &novel_id)
                                        .ok()?
                                        .iter()
                                        .filter_map(|p| {
                                            crud::list_outline_chapters(&conn, &p.id).ok()
                                        })
                                        .flatten()
                                ...
                                .ok_or_else(|| {
                                    "No chapter available. Create an outline chapter first."
                                        .to_string()
                                })?;
                ...
                None => {
                    ...
                    let msgs = vec![
                        pnw_kernel::agent::llm::Message {
                            ...
                            content:
                                "你是一个友善的 AI 助手，可以闲聊也可以帮助写作。当前没有打开项目。"
                                    .to_string(),
                        },
                        pnw_kernel::agent::llm::Message {
                            role: "user".to_string(),
...
fn get_stats(state: State<AppState>) -> Result<serde_json::Value, String> {
    ...
    for p in &outline_phases {
        if let Ok(chs) = crud::list_outline_chapters(conn, &p.id) {
            planned += chs.len() as i32;
        }
    }
...
fn list_characters(state: State<AppState>) -> Result<serde_json::Value, String> {
    ...
    let novel_id = active_novel_id(&handler.conn)?;
    let output = handler
        .execute(DataCommand::ListCharacters { novel_id })
        .map_err(|e| e.to_string())?;
    Ok(serde_json::to_value(output).unwrap_or_default())
}
...
fn get_setting(state: State<AppState>) -> Result<serde_json::Value, String> {
    ...
    let novel_id = active_novel_id(&handler.conn)?;
    let output = handler
        .execute(DataCommand::GetSetting { novel_id })
        .map_err(|e| e.to_string())?;
    Ok(serde_json::to_value(output).unwrap_or_default())
}
...
fn list_samples(state: State<AppState>) -> Result<serde_json::Value, String> {
    ...
    let novel_id = active_novel_id(&handler.conn)?;
    let output = handler
        .execute(DataCommand::ListSamples { novel_id })
        .map_err(|e| e.to_string())?;
    Ok(serde_json::to_value(output).unwrap_or_default())
}
...
fn get_plugin(state: State<AppState>) -> Result<serde_json::Value, String> {
    ...
    let novel_id = active_novel_id(&handler.conn)?;
    let output = handler
        .execute(DataCommand::GetPlugin { novel_id })
        .map_err(|e| e.to_string())?;
    Ok(serde_json::to_value(output).unwrap_or_default())
}
...
fn add_outline_chapter(
    ...
    let handler = ensure_handler(&state)?;
    let chapters =
        crud::list_outline_chapters(&handler.conn, &phase_id).map_err(|e| e.to_string())?;
...
fn add_character(
    ...
    let novel_id = active_novel_id(&handler.conn)?;
    let id = uuid::Uuid::new_v4().to_string();
    let cmd = DataCommand::CreateCharacter {
        id: id.clone(),
        novel_id,
        name,
        char_type,
